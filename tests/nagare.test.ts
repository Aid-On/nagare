...
describe('Nagare', () => {
  ...
  describe('Creation', () => {
    it('should create a nagare from array', async () => {
      const r = nagare.of(1, 2, 3, 4, 5);
      const result = await r.toArray();
      expect(result).toEqual([1, 2, 3, 4, 5]);
    });

    it('should create a nagare from range', async () => {
      const r = nagare.range(0, 5);
      const result = await r.toArray();
      expect(result).toEqual([0, 1, 2, 3, 4]);
    });

    it('should create an empty nagare', async () => {
      const r = nagare.empty<number>();
      const result = await r.toArray();
      expect(result).toEqual([]);
    });

    it('should create a nagare from promise', async () => {
      const r = nagare.fromPromise(Promise.resolve(42));
      const result = await r.toArray();
  ...
  describe('Operators', () => {
    it('should map values', async () => {
      const r = nagare.of(1, 2, 3).map(x => x * 2);
      const result = await r.toArray();
      expect(result).toEqual([2, 4, 6]);
    });

    it('should filter values', async () => {
      const r = nagare.of(1, 2, 3, 4, 5).filter(x => x % 2 === 0);
      const result = await r.toArray();
      expect(result).toEqual([2, 4]);
    });

    it('should scan values', async () => {
      const r = nagare.of(1, 2, 3).scan((acc, x) => acc + x, 0);
      const result = await r.toArray();
      expect(result).toEqual([1, 3, 6]);
    });

    it('should take first n values', async () => {
      const r = nagare.of(1, 2, 3, 4, 5).take(3);
      const result = await r.toArray();
      expect(result).toEqual([1, 2, 3]);
    });

    it('should skip first n values', async () => {
      const r = nagare.of(1, 2, 3, 4, 5).skip(2);
      const result = await r.toArray();
  ...
  describe('Fork and Merge', () => {
    ...
    it('should merge nagares', async () => {
      const r1 = nagare.of(1, 3, 5);
      const r2 = nagare.of(2, 4, 6);
  ...
  describe('Stream Conversion', () => {
    ...
    it('should create from ReadableStream', async () => {
      ...
      const r = Nagare.fromReadableStream(stream);
      const result = await r.toArray();
      expect(result).toEqual([1, 2, 3]);
    });
  });
